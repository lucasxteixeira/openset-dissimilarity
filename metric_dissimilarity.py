...
def compute_msp(probs):
  ...
  ----------
  probs : np.ndarray
      The predicted probabilities for the test data as an array of shape (n_samples, n_classes).

  Returns
  -------
  np.ndarray
      The maximum softmax probability for each test sample.
  """
...
def compute_mls(logits):
  ...
  ----------
  logits : np.ndarray
      The predicted logits for the test data as an array of shape (n_samples, n_classes).

  Returns
  -------
  np.ndarray
      The maximum logit score for each test sample.
  """
...
def compute_mds(X_train, X_test, Y_test=None, inverted=False):
  ...
  ----------
  X_train : np.ndarray
      The training data as a NumPy array of shape (n_samples, n_features).
  X_test : np.ndarray
      The test data as a NumPy array of shape (n_samples, n_features).
  Y_test : np.ndarray, optional
      The class labels for the test data.
      Used exclusively for dissimilarity vector representation.
      If provided, the test data is reshaped to match the number of samples.
      Defaults to None.
  inverted : bool, optional
      Whether to invert the dissimilarity scores. Defaults to False.
      If True, the scores are inverted to represent dissimilarity instead of similarity.
      This is useful for contrastive loss, where higher scores indicate more dissimilarity.
  ...
  # Scale the training data to [0, 1] range
  scaler = MinMaxScaler(clip=True).fit(X_train)
  X_test_scaled = scaler.transform(X_test)

  # Reshape the test data if Y_test is provided
  # This is needed for dissimilarity vector representation
  # The test data is reshaped to match the number of samples
  if Y_test is not None:
    X_test_scaled = np.reshape(X_test, (Y_test.shape[0], -1))

  # Flip scores when using an inverted similarity metric and collapse features to a single minimum dissimilarity score
  # Contrastive loss pushes similar samples to higher values and dissimilar samples to lower values with no bounds
  # Triplet loss works as intended, and still unbounded
  if inverted:
...
def closed_accuracy(preds, y_true, mask):
  ...
  y_true : np.ndarray
      The true class labels for the test data.
  mask : np.ndarray
      A boolean mask indicating which samples are known (True) or unknown (False).

  Returns
  -------
  float
      The closed-set accuracy for the known samples.
  """
...
def open_auroc(scores, known_mask, unknown_mask):
  ...
  ----------
  scores : np.ndarray
      The predicted scores for the test data.
  known_mask : np.ndarray
      A boolean mask indicating which samples are known (True) or unknown (False).
  unknown_mask : np.ndarray
      A boolean mask indicating which samples are unknown (True) or known (False).
...
def openauc(scores, predictions, y_true, known_mask, unknown_mask):
  ...
  -------
  float
      The OpenAUC score for open-set detection.
  """
  ...
  # Any misclassified known sample is forced to look like the worst unknown
  m_x2 = max(x2) + 1e-5

  y_score = [value if hit else m_x2 for value, hit in zip(x1, correct)] + x2
  y_true  = [0]*len(x1) + [1]*len(x2)  # 0 for known, and 1 for unknown
